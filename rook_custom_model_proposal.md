...
We will perform transfer learning on the base `yolo11n.pt` model. Since we are running on a Raspberry Pi 5, we must maintain the `n` (nano) architecture to keep the ~150ms inference time.
...
```
Deploy the resulting NCNN folder to `~/yolo11n_1088_ncnn_model` on the Pi.

...
2. **Auto-labeling** — `python3 app/llm_autolabel.py`: YOLO26l/x proposes boxes at conf 0.20, then a vision LLM classifies crops into `trash_truck`, `ups_truck`, `fedex_truck`, `amazon_van`, `usps_truck`, `baseball_player` (closed vocabulary, confidence-gated, content-hash cached so each crop is billed once)
